...
pub fn parse_geth_engine_error_hint(err: &str) -> Result<Option<EngineApiHint>, BuilderError> {
    debug!("Parsing Geth engine error hint: {}", err);
    let raw_hint_value = match REGEX.captures(err).and_then(|cap| cap.get(1)) {
        Some(matched) => matched.as_str().to_string(),
        None => {
            error!("No hint value found in Geth engine error hint: {}", err);
            return Ok(None);
        }
    };

    // Match the hint value to the corresponding hint type based on other parts of the error message
    if err.contains("blockhash mismatch") {
        return Ok(Some(EngineApiHint::BlockHash(B256::from_hex(raw_hint_value)?)));
    } else if err.contains("invalid gas used") {
        return Ok(Some(EngineApiHint::GasUsed(raw_hint_value.parse()?)));
    } else if err.contains("invalid merkle root") {
        return Ok(Some(EngineApiHint::StateRoot(B256::from_hex(raw_hint_value)?)));
    } else if err.contains("invalid receipt root hash") {
        return Ok(Some(EngineApiHint::ReceiptsRoot(B256::from_hex(raw_hint_value)?)));
    } else if err.contains("invalid bloom") {
    ...
    if err.contains("could not apply tx") {
        return Err(BuilderError::InvalidTransactions(err.to_string()));
    }

    Err(BuilderError::UnsupportedEngineHint(err.to_string()))
}
