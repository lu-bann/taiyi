...
fn empty_password() {
    assert_eq!(
        KeystoreBuilder::new(&Keypair::random(), "".as_bytes(), "".into()).err().unwrap(),
        Error::EmptyPassword
    );
}
...
fn string_round_trip() {
    ...
    let keystore =
        KeystoreBuilder::new(&keypair, GOOD_PASSWORD, "".into()).unwrap().build().unwrap();
...
fn file() {
    ...
    keystore.to_json_writer(File::create_new(&path).unwrap()).expect("should write to file");
...
fn scrypt_params() {
    ...
    // n <= 1
    let my_kdf = Kdf::Scrypt(Scrypt { dklen: DKLEN, n: 1, p: 1, r: 8, salt: salt.clone().into() });
    ...
    // p != 0
    let my_kdf = Kdf::Scrypt(Scrypt { dklen: DKLEN, n: 16, p: 0, r: 8, salt: salt.clone().into() });
    let keystore = KeystoreBuilder::new(&keypair, GOOD_PASSWORD, "".into())
        .unwrap()
        .kdf(my_kdf.clone())
        .build();
    ...
    // r != 0
    let my_kdf = Kdf::Scrypt(Scrypt { dklen: DKLEN, n: 16, p: 1, r: 0, salt: salt.clone().into() });
    let keystore = KeystoreBuilder::new(&keypair, GOOD_PASSWORD, "".into())
        .unwrap()
        .kdf(my_kdf.clone())
        .build();
...
fn custom_scrypt_kdf() {
    ...
    let my_kdf = Kdf::Scrypt(Scrypt { dklen: DKLEN, n: 2, p: 1, r: 8, salt: salt.clone().into() });
...
fn custom_pbkdf2_kdf() {
    ...
    let my_kdf =
        Kdf::Pbkdf2(Pbkdf2 { dklen: DKLEN, c: 2, prf: Prf::HmacSha256, salt: salt.clone().into() });
...
fn utf8_control_characters() {
    ...
    let keystore2 = KeystoreBuilder::new(&keypair, &password, "".into()).unwrap().build().unwrap();

    assert_eq!(keystore1.pubkey(), keystore2.pubkey());

    // Decode same keystore with nfc and nfkd form passwords
    let decoded1 = keystore1.decrypt_keypair(&password_with_control_chars).unwrap();
...
fn normalization() {
    ...
    // Decode same keystore with nfc and nfkd form passwords
    let decoded_nfc = keystore_nfc.decrypt_keypair(password_nfc.as_bytes()).unwrap();
    let decoded_nfkd = keystore_nfc.decrypt_keypair(password_nfkd.as_bytes()).unwrap();
