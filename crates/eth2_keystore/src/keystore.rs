...
impl Keystore {
    ...
    fn encrypt(
        ...
        Ok(Keystore {
            json: JsonKeystore {
                crypto: Crypto {
                    kdf: KdfModule { function: kdf.function(), params: kdf, message: EmptyString },
...
fn validate_parameters(kdf: &Kdf) -> Result<(), Error> {
    match kdf {
        ...
        Kdf::Scrypt(params) => {
            ...
            let mut npr: u32 = params.n.checked_mul(params.p).ok_or(Error::InvalidScryptParam)?;
...
fn validate_salt(salt: &[u8]) -> Result<(), Error> {
    ...
    if salt.is_empty() {
        ...
        eprintln!("WARN: Salt is too short {}, we recommend {}", salt.len(), SALT_SIZE);
    } else if salt.len() > SALT_SIZE * 2 {
        eprintln!("WARN: Salt is too long {}, we recommend {}", salt.len(), SALT_SIZE);
    }
    Ok(())
}
