...
pub fn verify_signature_sets<'a>(
    ...
    for set in &sets {
        ...
        let signing_keys = set.signing_keys.iter().map(|pk| pk.point()).collect::<Vec<_>>();
...
impl Clone for BlstAggregatePublicKey {
    fn clone(&self) -> Self {
        Self(blst_core::AggregatePublicKey::from_public_key(&self.0.to_public_key()))
    }
}
...
impl Clone for BlstAggregateSignature {
    fn clone(&self) -> Self {
        Self(blst_core::AggregateSignature::from_signature(&self.0.to_signature()))
    }
}
