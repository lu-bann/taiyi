...
pub trait PreconfPricer {
    ...
    {
        async move {
            let current_base_fee: u64 = self.get_optimal_base_gas_fee().await?;
...
impl PreconfPricer for TaiyiFeePricer {
    async fn get_optimal_base_gas_fee(&self) -> eyre::Result<u64, PricerError> {
        let response = reqwest::get(self.url.clone()).await?;
        let body = response.bytes().await?;

        let body_str = String::from_utf8_lossy(&body);
        let res = body_str.parse::<u64>().map_err(|e| PricerError::ParseError(e.to_string()))?;
        Ok(res)
    }
}
